...
@rem ====================================================================== ::
@rem                                                                          
@rem  Duckula-kafka-consumer 任务启动脚本	
@rem  %1 consumerId
@rem  %2 jmx port
@rem  %3  debug port
@rem                                                                          
...
@rem 切换盘符
d:
cd %DUCKULA_DATA%/logs/consumer
if not exist %DUCKULA_DATA%/logs/consumer/%1 (  
   mkdir %1
) 
...
@rem 远程调试
if not "%3"=="" (
  set "JAVA_DEBUGGER=-Xdebug -Xnoagent -Djava.compiler=NONE -Xrunjdwp:transport=dt_socket,server=y,suspend=y,address=%3"
)       
...
set "LOG_ROOT=-DlogRoot=%DUCKULA_DATA%/logs/consumer/%1"

set JAVA_OPTS=%JAVA_MEM_OPTS% %JAVA_JMX% %JAVA_DEFAULT_OPT% %LOG_ROOT% %JAVA_DEBUGGER%
echo  cmdstr="java  %JAVA_OPTS%  -jar %DUCKULA_HOME%/duckula-kafka-consumer.jar  %1"
echo "============================serverice started============================================="
@rem  start javaw  %JAVA_OPTS%  -jar %DUCKULA_HOME%/duckula-kafka-consumer.jar  %1
